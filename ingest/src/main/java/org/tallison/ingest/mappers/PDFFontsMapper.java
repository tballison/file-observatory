...
public class PDFFontsMapper implements FeatureMapper {
    @Override
    public void addFeatures(Map<String, String> row, Fetcher fetcher, StoredDocument storedDocument)
            throws SQLException {
        String stdout = row.get("pdffonts_stdout");
        String stderr = row.get("pdffonts_stderr");
        storedDocument.addNonBlankField("pf_stderr", ESUtil.stripIllegalUnicode(stderr));
        storedDocument.addNonBlankField("pf_stdout", ESUtil.stripIllegalUnicode(stdout));
        if (stdout == null) {
            return;
        }
        ...
        storedDocument.addNonBlankField("pf_font_names", toList(names));
        storedDocument.addNonBlankField("pf_font_types", toList(types));
        storedDocument.addNonBlankField("pf_font_encodings", toList(encodings));
        storedDocument.addNonBlankField("pf_fonts_embedded", multiBinary(embedded));
        storedDocument.addNonBlankField("pf_fonts_uni", multiBinary(unicode));
    }
